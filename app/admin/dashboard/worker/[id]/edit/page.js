...
export default function EditWorker({ params }) {
  ...
  useEffect(() => {
    const fetchWorker = async () => {
      const { data, error } = await supabase.from('workers').select('*').eq('id', unwrappedParams.id).single();
      if (data) {
        setFormData(data);
      } else if (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await supabase.from('workers').update(formData).eq('id', unwrappedParams.id);
  ...
  if (fetching) return <div className="flex justify-center py-20"><Loader2 className="w-10 h-10 text-indigo-600 animate-spin" /></div>;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-8">Edit Worker</h1>
